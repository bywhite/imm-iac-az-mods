...
resource "intersight_vnic_lan_connectivity_policy" "lancon_2" {
  name                = "${var.policy_prefix}-lancon-2"
...
resource "intersight_vnic_eth_if" "eth0_2" {
  name             = "eth0"       #"${var.policy_prefix}-eth0-2"  #Need unique index per netcon
  ...
  lan_connectivity_policy {   # Groups eth[*] interfaces together and sets FI-attached
    moid        = intersight_vnic_lan_connectivity_policy.lancon_2.id
...
resource "intersight_vnic_eth_if" "eth1_2" {
  name             = "eth1"    #"${var.policy_prefix}-eth1-2"  #Need unique index per netcon
